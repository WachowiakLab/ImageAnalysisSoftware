...
%               'CloseRequestFcn',@hideToolbox,'Resize','off','Color', get(0,'DefaultUicontrolBackgroundColor'));
            ...
            threshold = uicontrol('Parent',fig,'style','edit','String','0.5','HorizontalAlignment','left',...
                'Units','pixels','Position',[330 38 30 20],'BackgroundColor',[1 1 1]);
            checkbox_1 = uicontrol('Parent',fig,'Style','checkbox','Units','pixels','Position',[163 8 15 15],...
                'String','Revert ?','Min',0,'Max',1,'Callback',@revert_sniff_trace_Callback);
            uicontrol('Parent',fig,'Style','text','String','Revert ?','Units','pixels','Position',[120 2 43 23]);
            radiogroup = uibuttongroup('Parent',fig,'Units','pixels','Position',[10 80 380 40]);
            radio1 = uicontrol('Parent',radiogroup,'Units','pixels','Position',[170 10 90 20],...
                'Style','Radio','String','Zero-X'); 
            radio2 = uicontrol('Parent',radiogroup,'Units','pixels','Position',[270 10 90 20],...
                'Style','Radio','String','Trough'); %#ok<NASGU>
            uicontrol('Parent',radiogroup,'Style','text','String','Choose detection method :','Units','pixels','Position',[10 8 150 20])
            set(radiogroup,'SelectedObject',radio2);
            set(radiogroup,'SelectionChangeFcn',@selcbk);
            det_meth = get(radio2,'String');
            ...
            refresh_current_trial_details
            revert_default=get(checkbox_1,'Min');
            set(checkbox_1,'Value',revert_default);
            revert = revert_default;
         ...
         function selcbk(~,eventdata)
             det_meth = get(eventdata.NewValue,'String');
         end
         
%        revert=get(checkbox_1,'Value');
       function revert_sniff_trace_Callback(varargin)
            if get(checkbox_1,'Value') == 1
                revert=1;
            else
                revert=0;
            end
        ...
        function auto_detect_stim_times_Callback(varargin)
            ...
            for ti = 1:length(t)
                ...
                elseif isfield(o.trials(t(ti)).other,'sniff_pressure')
                    [stims{ti}, params] = custom_autodetect_stims('sniff_pressure',o.trials(t(ti)).other.sniff_pressure,o.trials(t(ti)).other.samplingrate,thresh,rev,detect);
                elseif isfield(o.trials(t(ti)).other,'sniff_thermo')
                    [stims{ti}, params] = custom_autodetect_stims('sniff_thermo',o.trials(t(ti)).other.sniff_thermo,o.trials(t(ti)).other.samplingrate,thresh,rev,detect);
                ...
                if true || ~isfield(o.trials(t(ti)).measurement,'stim_times') || ...
        ...
        function test_stim_times_Callback(varargin)
            ...
            elseif isfield(o.trials(t).other,'sniff_thermo')
                custom_detect_stims('sniff_thermo',o.trials(t).other.sniff_thermo,o.trials(t).other.samplingrate,thresh,rev, detect)
...
function [stimtimes params] = custom_autodetect_stims(type, tr, samplingrate, thresh,revrt,det)
    %fcn = str2func(['stimtime_' type]);
    fcn=str2func('stimtime_sniff_thermo');
    if strcmp(type,'sniff_thermo')
        [~, ~, stimtimes] = fcn(tr, samplingrate, thresh,revrt,det);
    else
        [~, ~, stimtimes] = fcn(tr, samplingrate, thresh,revrt,det);
    end
...
function custom_detect_stims(type, tr, samplingrate, thresh,revrt,det)
    if which(['stimtime_' type])
        open(['stimtime_' type])
        
        fcn = str2func(['stimtime_' type]);
        if strcmp(type,'sniff_thermo')
            [display_trace, horz_lines, stimtimes] = fcn(tr, samplingrate, thresh,revrt,det);
        else
