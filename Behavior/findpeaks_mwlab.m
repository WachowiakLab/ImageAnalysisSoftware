function [Ypk,Xpk,Wpk,Ppk] = findpeaks_mwlab(hAx,times,Yin,varargin)
%function [Ypk,Xpk,Wpk,Ppk] = findpeaks_mwlab(hAx,Yin,varargin)
%copied from MATLAB Feb. 2019. two input arguments (hAx,times) added for mwlab version
...
%   hAxes = plotSignalWithPeaks(x,y,iPk);
  hAxes = plotSignalWithPeaks(hAx,times,x,y,iPk);
  if strcmp(annotate,'extents')
    plotExtents(hAxes,times,x,y,iPk,bPk,bxPk,byPk,wxPk,refW);
  end
...
function hAxes = plotSignalWithPeaks(hAx,times,x,y,iPk)

% plot signal
% hLine = plot(x,y,'Tag','Signal');
hLine = plot(hAx,times(x),y,'Tag','Signal');
...
%--------------------------------------------------------------------------
function plotExtents(hAxes,times,x,y,iPk,bPk,bxPk,byPk,wxPk,refW)
...
if strcmp(refW,'halfheight')
  % plot height
  plotLines(hAxes,'Height',times(x(iPk)),y(iPk),times(x(iPk)),zeros(length(iPk),1),colors(2,:));  

  % plot width
  plotLines(hAxes,'HalfHeightWidth',times(wxPk(:,1)),hm,times(wxPk(:,2)),hm,colors(3,:));
      
  % plot peak borders
  idx = find(byPk(:,1)>0);
  plotLines(hAxes,'Border',times(bxPk(idx,1)),zeros(length(idx),1),times(bxPk(idx,1)),byPk(idx,1),colors(4,:));
  idx = find(byPk(:,2)>0);
  plotLines(hAxes,'Border',times(bxPk(idx,2)),zeros(length(idx),1),times(bxPk(idx,2)),byPk(idx,2),colors(4,:));
  
else
  % plot prominence
  plotLines(hAxes,'Prominence',times(x(iPk)), y(iPk), times(x(iPk)), bPk, colors(2,:));  
  
  % plot width
  framerate = diff(times(1:2));
  plotLines(hAxes,'HalfProminenceWidth',wxPk(:,1)*framerate, hm, wxPk(:,2)*framerate, hm, colors(3,:));
  
  % plot peak borders
%   idx = find(bPk(:)<byPk(:,1));
%   plotLines(hAxes,'Border',times(bxPk(idx,1)),bPk(idx),times(bxPk(idx,1)),byPk(idx,1),colors(4,:));
%   idx = find(bPk(:)<byPk(:,2));
%   plotLines(hAxes,'Border',times(bxPk(idx,2)),bPk(idx),times(bxPk(idx,2)),byPk(idx,2),colors(4,:));
end
