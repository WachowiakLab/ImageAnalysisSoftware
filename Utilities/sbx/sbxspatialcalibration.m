...
for mag = 1:13
    ...
    cal(mag).x = delta(mag)/v;
    cal(mag).y = delta(mag)/u;
    cal(mag).gain_resonant_mult = sbconfig.gain_resonant_mult;          % same for all (for now...)
end

fclose('all');                                    % to close file descriptors
global info
info = [];
...
fx = fit(mag,x,'exp2');
fig = figure('MenuBar','None','NumberTitle','off','Name','Spatial Calibration');
fig.Position = [693   494   884   350];
subplot(1,2,1);
plot(fx,mag,x','o')
xlabel('Magnification');
ylabel('Pixel width [um]');
legend off 
box off;
...
warndlg({sprintf('Calibration for %s complete and saved!',objective_h.String{objective_h.Value});...
    sprintf('Optimal value of gain_resonant_mult is %.3f to achieve aspect ratio = 1',b*sbconfig.gain_resonant_mult); ...
    'You must re-calibrate for each objective if you decide to change it';
    'Calibration will take effect after restarting Scanbox'});

objective_h.Callback(objective_h,[]);               % to set the spatial mag values for new calibration
