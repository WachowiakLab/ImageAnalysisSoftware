...
% Last Modified by GUIDE v2.5 30-Jul-2018 13:09:33
...
if(exist(fref,'file'))
    ...
    drawnow;
    %original code, uncomment these two lines to recover.
%     ref = double(intmax('uint16'))*size(mm0.Data.img,4)-sum(mm0.Data.img,4,'double');
%     ref = permute(ref,[1 3 2]);
    
    
    % ref = squeeze(ref(pmt,:,:));
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%here try to make 'smart' ref image by excluding frames outside 1 SD of mean. by mw
    %assignin('base','mm0',mm0);  %use to test
    meansig=squeeze(mean(mm0.Data.img,[2,3]));
    normsig=normalize(meansig,'zscore');   
    goodinds=find(abs(normsig)<=1);
    goodmm0=mm0.Data.img(:,:,:,goodinds);
    size(goodmm0)
    ref = double(intmax('uint16'))*size(goodmm0,4)-sum(goodmm0,4,'double');
    ref = permute(ref,[1 3 2]);
    clear('goodmm0');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%end MW mod part
    clear('mm0');
...
%%%%%%%%%%%added by MW - create copies of .ephys and .realtime files with
%%%%%%%%%%%'rigid' in the name
fephys=sprintf('%s.ephys',fname);
frealtime=sprintf('%s_realtime.mat',fname);
if(exist(fephys,'file'))
    copyfile(fephys, sprintf('a_%s.ephys',fname));
    assignin('base','ephysfound','yes');
else
    assignin('base','ephysfound','no');
end
if(exist(frealtime,'file'))
    copyfile(frealtime, sprintf('a_%s_realtime.mat',fname));
    assignin('base','realtimefound','yes');
else
    assignin('base','realtimefound','no');
end
...
for w = 1:nf
    ...
    try
        [~,~] = system(sprintf('fsutil file createnew a_%s.sbx %d',fname,d(1).bytes));
    catch
    end
    
    [~,~] = system(sprintf('copy %s.mat a_%s.mat',fname,fname));
    
    mm0 = sbxreadmmap(fname);           % original
    mm  = sbxreadmmap(fname, 'a_');  % aligned
...
%     
    ...
    for p = 1:npass
        ...
        for i = 1:bsz:(max_idx)
            ...
            for j=1:M
                t{j} = fftalign(squeeze(z(pmt,yidx,xidx,j)),ref(yidx,xidx),handles.imgf.Value);
            end
    ...
    m = ref;
    %save([fname '.sbxalign'],'m','T');    
    save(['a_', fname, '.sbxalign'],'m','T');
...
function r = fftalign(A,B,varargin)
...
switch flag
    case 1 % for aligning with cells
        A = filter2(fspecial('log',5,2),A,'valid');
        B = filter2(fspecial('log',5,2),B,'valid');

    case 2  % align with blood vessels
        A = imgaussfilt(A,5);
        B = imgaussfilt(B,5);
%         A = filter2(fspecial('gauss',15,5),A,'valid');
%         B = filter2(fspecial('gauss',15,5),B,'valid');
end
...
% --- Executes on selection change in imgf.
function imgf_Callback(hObject, eventdata, handles)
% hObject    handle to imgf (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns imgf contents as cell array
%        contents{get(hObject,'Value')} returns selected item from imgf


% --- Executes during object creation, after setting all properties.
function imgf_CreateFcn(hObject, eventdata, handles)
% hObject    handle to imgf (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
