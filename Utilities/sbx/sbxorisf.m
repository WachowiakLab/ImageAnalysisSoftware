...
% average across spatial phase and compute f1f0
...
for(t=1:ntau)
    for(n = 1:ncell)
        ...
        rf = squeeze(r(:,:,t,n)); % padding...
        rf2 = [rf(end-1,:); rf(end,:); rf ;rf(1,:) ; rf(2,:)]; % circular
...
ori = 0:10:170;
sper = 1920./(1.31.^(0:11));   % spatial period in pixels - should match what we have in objectview script
sper_pix = sper;
...
% calculate for each case... (each cell)
...
for i = 1:size(r,4)
    ...
    stat(i).f1f0 = f1f0(idx_ori,idx_sf,tmax,i);
