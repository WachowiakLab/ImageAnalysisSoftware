function newmap = bluewhitered_auto(varargin)
%BLUEWHITERED_AUTO Blue-White-Red color map. This is a diverging colormap,
%   useful for showing data with positive values in red, negative values
%   in blue, and values of zero in white.
%
%   BLUEWHITERED_AUTO(M,CLIM) returns an M-by-3 matrix containing a colormap. 
%   The colors may range from blue increasing through white to red.
%   Uses CLIM property to automatically scale maximal red/blue shading.
%
%   BLUEWHITERED_AUTO(M) returns an M-by-3 matrix containing a colormap. 
%   The colors may range from blue increasing through white to red.
%   Uses current axes CLIM property to automatically scale maximal red/blue
%   shading. If no axes exists, CLIM = [-1 1].
%
%   BLUEWHITERED_AUTO returns a colormap with the same number of colors as
%   the current figure's colormap. If no figure exists, MATLAB uses the
%   length of the default colormap. The colors may range from blue
%   increasing through white to red. Uses current axes CLIM property to
