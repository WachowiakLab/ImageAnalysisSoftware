function newmap = nawhimar_automax(varargin)
...
%
%   NAWHIMAR_AUTOMAX(M,CLIM) returns an M-by-3 matrix containing a colormap. 
%   The colors range from navy(dark blue) increasing through azure 
...
%   shading.
%
%   NAWHIMAR_AUTOMAX(M) returns an M-by-3 matrix containing a colormap. 
%   The colors range from navy(dark blue) increasing through azure 
%   (bright blue) to white, continuing through bright red to maroon
%   (dark red). Uses current axes CLIM property to automatically scale
%   maximal red/blue shading. If no axes exists, CLIM = [-1 1].
%
%   NAWHIMAR_AUTOMAX returns a colormap with the same number of colors as
%   the current figure's colormap. If no figure exists, MATLAB uses the
%   length of the default colormap. The colors range from navy (dark
