...
function CBAvgSelected(~,~)
    ...
    for nn = 1:length(tmpval)
        ...
        header = fread(fid(nn), 2880, 'uint8'); %this is size of header of .tsm file in uints
    ...
    delete(tmpbar);
    im = im./length(tmpval);  %this does the averaging...
    
    [~,origname,~] = fileparts(filenames{1});
    origtbnfile=fullfile(filepath,[origname,'.tbn']);
    disp(origtbnfile);
    ...
    [newfilename,newpath]=uiputfile('*.tsm','Save Averaged SM64 File',tmpname);
    %first, just copy the tbn (BNC) file with the new name
    newtbnfile= fullfile(newpath,[tmpname,'.tbn']);
    disp(newtbnfile)
    [status, msg] = copyfile(origtbnfile, newtbnfile);
    %%%%%%%%%%%%%
    nfid = fopen(fullfile(newpath,newfilename),'w');
    %fwrite(nfid,[header;im],'int16'); old version, for NP
    fwrite(nfid,header,'uint8');
    fwrite(nfid,im,'uint16');
    fclose(nfid);
...
function CBAvgSelectedByOdor(~,~)
    ...
    tmpval = h.filelist.Value;
    newname = []; newheader = []; newdat = []; cnt = []; newodornum = []; origfilename=[];
    tmpbar = waitbar(0,'Averaging Selected Files by Odor');
    for n = 1:length(tmpval)
        waitbar(n/length(tmpval),tmpbar);
        fid(n) = fopen(fullfile(filepath,filenames{tmpval(n)}));        
        header = fread(fid(n), 2880, 'uint8'); %this is size of header of .tsm file in uints
        dat = fread(fid(n), inf, 'int16');
        fclose(fid(n));
        tmpname = h.odornamelist.String{tmpval(n)};
        tmpodornum = str2num(h.odornumlist.String{tmpval(n)});
        tmpidx = find(strcmp(newname,tmpname));
        if isempty(tmpidx)   %this happens the first time a given odorname is searched.
            newname{end+1} = tmpname;            
            newodornum{end+1} = tmpodornum;
            idx(n) = length(newname); 
            newheader{end+1} = header;  %not sure what this does.....
            newdat{end+1} = dat;
            origfilename{end+1}=filenames{tmpval(n)};
            cnt(end+1) = 1;
        else
            idx(n) = tmpidx; 
    ...
    for i = 1:length(newname)
        [~,origname,~] = fileparts(origfilename{i});
        origtbnfile=fullfile(filepath,[origname,'.tbn']);
        disp(origtbnfile);
        newtbnfile=fullfile(newpath,[nameprefix newname{i} '.tbn']);
        disp(newtbnfile);
        [status, msg] = copyfile(origtbnfile, newtbnfile); %%%copy to new .tbn file
        newdat{i} = newdat{i}./cnt(i);
        newname{i} = fullfile(newpath,[nameprefix newname{i} '.tsm']);
       nfid(i) = fopen(newname{i},'w');
        fwrite(nfid(i),newheader{i},'uint8');
        fwrite(nfid(i),newdat{i},'uint16');
