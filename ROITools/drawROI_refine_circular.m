...
%function rois = drawROI_refine(rois,bgimage,imstack,varargin)
%   optional: varargin{1} is a colormap, varargin{2} is caxis values
if verLessThan('matlab','9.5') %revert to pre-MATLAB 2018b code (no drawpolygon.m & older zoom tools)
    if nargin>3
        if numel(varargin) == 1
            rois=drawROI_refine_pre2018b(rois,bgimage,imstack,varargin{1}); waitfor(rois);
        elseif numel(varargin) == 2
            rois=drawROI_refine_pre2018b(rois,bgimage,imstack,varargin{1},varargin{2}); waitfor(rois);
        else
            disp('Incorrect number of inputs to drawROI_refine'); return;
        end
    else
        rois=drawROI_refine_pre2018b(rois,bgimage,imstack); waitfor(rois);
    end
else
...
% create ROI figure
global hROI ROIdata; %should get rid of global variables!
...
function drawBG %draw background image
    ...
    hROI.Zoom = zoom(hROI.newaxes); zoom reset;
    hROI.Pan = pan;
...
%     hROI.newaxes.XLimMode = 'manual'; hROI.newaxes.YLimMode = 'manual'; hROI.newaxes.ZLimMode = 'manual';
    set(hROI.newaxes, 'Position', [0 0 1 .88]);
    try
    ...
    if hROI.autoCaxis.Value
        tmplim = prctile(ROIdata.bgimage(:),[0.2 99.8]);
        if tmplim(2) == tmplim(1); tmplim(2) = tmplim(1)+1; end %just in case the image is all zeros    
        hROI.newaxes.CLim = tmplim;
        hROI.cmin_edit.String = sprintf('%5.3f',tmplim(1));
        hROI.cmax_edit.String = sprintf('%5.3f',tmplim(2));
    else 
        hROI.newaxes.CLim = [str2double(hROI.cmin_edit.String) str2double(hROI.cmax_edit.String)];
    end
    CBsetClim;
...
function CBsetClim(~,~)
    ...
    hROI.stepsize = (hROI.th_max-hROI.th_min)/100;
    thresh = str2double(hROI.thresh_edit.String);
...
function CBpolygon(~,~)
    ...
    Zon=0; if strcmp(hROI.Zoom.Enable,'on'); Zon=1; hROI.Zoom.Enable='off'; end
    if strcmp(hROI.Pan.Enable,'on'); hROI.Pan.Enable='off'; end
...
%     hROI.fig.WindowButtonMotionFcn=[];
    ...
    newROI = drawpolygon(hROI.newaxes);
    if ~exist('newROI','var') || length(newROI.Position)<3
        delete(newROI);
    else
...
%         hROI.fig.WindowScrollWheelFcn=[];
        ...
        if hROI.quickmethod.Value
            ...
            hROI.newaxes.Toolbar.Visible='on';
        end
    end
end
...
function CBScroll(~,cbdata)
    ...
    if z(1)>hROI.newaxes.XLim(1) && z(2)>hROI.newaxes.YLim(1) && z(1)<hROI.newaxes.XLim(2) && z(2)<hROI.newaxes.YLim(2)
        ...
        if hROI.invert_thresh.Value
            thresh = str2double(hROI.thresh_edit.String)+cbdata.VerticalScrollCount*hROI.stepsize;
            if thresh>hROI.th_max; thresh=hROI.th_max; end
            if thresh<hROI.th_min; thresh=hROI.th_min; end
        ...
        %hROI.newmask = bw; %use this if not using imfill below
...
function CBClick(~,~)
    if max(hROI.newmask(:))>0
        ...
        xl=hROI.newaxes.XLim; yl=hROI.newaxes.YLim;
        drawBG;
        hROI.newaxes.XLim=xl; hROI.newaxes.YLim=yl; drawnow;
        CBthreshold;
    end
end
function CBQuickClick(~,~)
    persistent newcontour;
%     pause(.01); if strcmp(hROI.Zoom.Enable,'on'); hROI.Zoom.Enable='off'; return; end
    if exist('newcontour','var'); delete(newcontour); end
    z = round(hROI.newaxes.CurrentPoint);
    z = z(1,1:2);
...
%         xstart = max(1,z(1)-floor(L/2)); xstart = min(z(1)-floor(L/2),hROI.newaxes.XLim(2)-L);
  ...
  % try to make circular mask instead      
   % Calculate the distance of each point from the center
      ...
      distanceFromCenter = sqrt((imgdims(1) - z(1)).^2 + (imgdims(2) - z(2)).^2);
% Create the binary mask
    hROI.newmask = distanceFromCenter <= L;
   %%%%%%%%%%%%%%%%%%%%%%%
...
function outmask = refine(inmask) %Refine the ROI
    ...
    if strcmp(answr, 'Draw New ROI') %Define New
        delete(incontour);
        newROI = drawpolygon(hROI.newaxes, []);
        if isempty(newROI) || isempty(newROI.Position); delete(newROI); outmask=inmask; return; end
