function hTS = TimeSeriesAnalysis_MWLab_roi2sniff(varargin)
...
%     stimstr = {'-Select Auxiliary Signal-';'Aux1(odor)';'Aux2(sniff)';'AuxCombo';'Manually Defined Stimulus'};
    ...
    uicontrol(tab,'Tag','subtractbgroi','Style', 'checkbox', 'Units', 'normalized', 'Position', ...
        [0.02 0.285 0.42 0.03], 'String', 'Use ROI # for sniff (irreversible!):', 'Value', bgroival, ...
...
function [allOdorTrials, preplotdata] = getPlotData
    ...
    for f = 1:length(files)
        ...
        %replace ephys sniff signal with roi if chcecked.
        if get(findobj(tab,'Tag','subtractbgroi'),'Value')
            bgroinum = str2double(get(findobj(tab,'Tag','bgroi'), 'String'));
            bgseries=TSdata.file(files(f)).roi(bgroinum).series;
            bgtimes=TSdata.file(files(f)).roi(bgroinum).time;
            sniffsig=preplotdata.file(f).ephys.sniff;
            snifftime=preplotdata.file(f).ephys.times; %note that ephys signal starts earlier than optical sig, so includes negative times
            %%%new code to replace sniff signal with chosen ROI signal
            preinds=find(snifftime<0);
            postinds=find(snifftime>max(bgtimes));
            trimmedsnifftimes=snifftime(max(preinds)+1:postinds(1)-1);
            trimmedsniffsig=sniffsig(max(preinds)+1:postinds(1)-1);
            roisniffsig=interp1(bgtimes, bgseries, trimmedsnifftimes,'pchip');
            roisniffsig=roisniffsig-mean(roisniffsig);  %subtract mean if this will be sniff signal.
            newsniffsig=sniffsig;
            newsniffsig(1:max(preinds))=0;
            newsniffsig(max(preinds)+1:postinds(1)-1)=roisniffsig;
            newsniffsig(postinds(1):end)=0;
            preplotdata.file(f).ephys.sniff=newsniffsig;
            hTS.UserData.TSdata.file(f).ephys.sniff=newsniffsig;
            %assignin('base','newsniffsig',newsniffsig);
            clear sniffsig snifftime trimmedsnifftimes trimmedsniffsig newsniffsig;
            %%%%%%%%%%%%%%%%
%             for r = 1:length(rois)
...
%                             preplotdata.file(f).avgroi.odor(oo).trial(ttt).confint(2,:) = preplotdata.file(f).avgroi.confint(2,index);
                        ...
                        if strcmp(get(findobj(tab,'Tag','ephys'),'Visible'), 'on')
                            ...
                            %assignin('base',"snifftimes",preplotdata.file(f).ephys.odors(oo).trials(ttt).times);
                            ...
                            %assignin('base',"sniffsig",preplotdata.file(f).ephys.odors(oo).trials(ttt).sniff);
